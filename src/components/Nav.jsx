    ...
    <header className="padding-x py-4 absolute z-10 w-full ">
      <nav className=" flex justify-between item-center  max-container">
        ...
        <div className=" max-lg:flex hidden flex-col justify-end items-center ">
          ...
          />
          <div className={`${!toggle ? 'hidden' : 'flex' } absolute top-0 left-0 z-10 mt-[60px] bg-orange-200 w-full h-[450px] md:h-[400px]  `}>
            <div className=" flex items-center w-full h-full">
            <ul className=" w-full flex flex-col gap-10 justify-center items-center">
              {navLinks.map((items) => (
                <li key={items.label}>
                  <a href={items.href} className=" hover:text-slate-gray font-montserrat leading-normal text-2xl font-semibold text-black">
                    {items.label}
                  </a>
                </li>
              ))}
